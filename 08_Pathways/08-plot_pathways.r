...
res_padjfilt = vroom::vroom("~/MinaRyten/Melissa/teamWood/ClusterProfiler/07-clusterProfiler_res_logfc1.csv", show_col_types = FALSE) %>% 
  janitor::clean_names() %>% 
  ...
  dplyr::left_join(
    ...
    y=plot_data %>% 
      ...
      dplyr::mutate(pathway_order = case_when(
        F < 0.05 & C < 0.05 ~ 4,      # both significant
        F < 0.05 & C > 0.05 ~ 3,      # only F significant
        C < 0.05 & F > 0.05 ~ 2,      # only C significant
        TRUE ~ 0 )) %>% 
      dplyr::mutate(pathway_order_label = case_when(
      #  pathway_order == 5 ~ "2/3 tissues",
        pathway_order == 4 ~ "Cingulate and Frontal",
      ...
      dplyr::mutate(n_assays_sig_for_pw = sum(`F`<0.05) + sum(`C`<0.05)) %>% 
      dplyr::ungroup() %>% 
      dplyr::select(-`F`, -`C`) %>% 
